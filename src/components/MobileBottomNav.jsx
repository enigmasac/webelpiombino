...
function WhatsAppPanel({ onClose }) {
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[60] bg-[#075E54] flex flex-col"
    >
      <div className="flex items-center justify-between px-6 pt-14 pb-4">
        <h2 className="font-heading text-2xl text-white">WhatsApp</h2>
      ...
      <p className="px-6 text-white/60 text-sm mb-6">
        Selecciona el local con el que deseas comunicarte.
      </p>

      <div className="flex-1 px-6 space-y-4 overflow-y-auto pb-8">
        {locations.map((loc, idx) => {
          const phone = loc.phone.replace(/\s/g, "");
          return (
            <motion.a
              key={loc.id}
            ...
            >
              <h3 className="font-heading text-xl text-[#075E54] mb-1">
                {loc.name}
              </h3>
              <p className="text-brown/60 text-sm mb-1">{loc.address}</p>
              <p className="text-brown/40 text-xs mb-4">{loc.phone}</p>
              <span className="inline-flex items-center gap-2 bg-[#25D366] text-white font-body text-sm font-semibold uppercase tracking-wider px-6 py-2.5 rounded-full">
                Abrir chat
...
function MenuCard({ item, onClick, onWhatsApp }) {
  ...
  if (item.isWhatsApp) {
    return (
      <button className={cls} onClick={() => { onClick(); onWhatsApp(); }}>
      ...
      <div className={`fixed inset-x-0 bottom-0 z-[45] lg:hidden transition-transform duration-300 ease-out ${menuOpen ? "translate-y-0" : "translate-y-full"}`}>
        <div className="bg-cream border-t border-line rounded-t-3xl pb-24 pt-6 px-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {menuItems.map((item) => (
              <MenuCard key={item.label} item={item} onClick={() => setMenuOpen(false)} onWhatsApp={() => setWhatsAppOpen(true)} />
            ))}
          </div>
        </div>
      </div>
